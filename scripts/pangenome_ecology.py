#!/usr/bin/env python3
"""Quantify pangenome sharing among ecological or metadata-defined groups.

PANOPTICON ecology module
-----------------
...
4. Pairwise heatmaps, an UpSet plot, and a group-sharing network.
...
def assemble_ecology_summary_figure(
    stratification_plot: Path,
    plots_dir: Path,
    output_path: Path,
) -> None:
    """Assemble principal ecology outputs into a four-panel figure."""
    try:
        from PIL import Image, ImageDraw, ImageFont
    except ImportError as exc:
        raise RuntimeError(
            "Pillow is required for --summary-figure. Install with: "
            "conda install -c conda-forge pillow"
        ) from exc

    panel_paths = [
        stratification_plot,
        plots_dir / "heatmap_jaccard_rarefied.png",
        plots_dir / "heatmap_group_core_jaccard.png",
        plots_dir / "private_gene_family_counts.png",
    ]
    missing = [str(path) for path in panel_paths if not path.is_file()]
    if missing:
        raise FileNotFoundError(
            "Cannot assemble summary figure; missing panels: " + ", ".join(missing)
        )

    images = [Image.open(path).convert("RGB") for path in panel_paths]
    cell_w, cell_h = 2400, 1800
    margin, gap, label_band = 110, 70, 100
    canvas = Image.new(
        "RGB",
        (margin * 2 + cell_w * 2 + gap,
         margin * 2 + (cell_h + label_band) * 2 + gap),
        "white",
    )
    ...
    for index, image in enumerate(images):
        ...
        y = margin + row * (cell_h + label_band + gap)
        ...
        py = y + label_band + (cell_h - resized.height) // 2
        canvas.paste(resized, (px, py))
        draw.text((x, y), chr(65 + index), fill="black", font=font)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--min-group-family-count", "--min-host-count", dest="min_host_count", type=int, default=2)
    parser.add_argument("--min-group-family-prevalence", "--min-host-prevalence", dest="min_host_prevalence", type=float, default=0.01)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Assemble a four-panel ecology summary figure after plotting. "
            "Panels: stratified accumulation, rarefied Jaccard similarity, "
            "group-core similarity, and ecology-specific family counts."
        ),
    )
    parser.add_argument(
        "--stratification-plot",
        help=(
            "Path to the ecology rarefaction/accumulation PNG generated by "
            "pangenome_host_association.py. Required with --summary-figure."
        ),
    )
...
def main() -> None:
    ...
    if args.summary_figure:
        if not args.stratification_plot:
            raise ValueError("--summary-figure requires --stratification-plot.")
        assemble_ecology_summary_figure(
            stratification_plot=Path(args.stratification_plot),
            plots_dir=plots_dir,
            output_path=outdir / "ecology_summary_figure.png",
        )
    ...
    manifest = {
        ...
        "summary_figure": bool(args.summary_figure),
        "stratification_plot": args.stratification_plot,
